...
#
# Tags look like `v<Nextcloud version>-<release>`:
#
# - if defaults/main.yml points at a Nextcloud version that has never been
#   released, the release counter restarts at 0 (`v34.0.3-0`)
# - otherwise the counter is incremented (`v34.0.3-1`), but only if something
...
version="$(sed -nE 's|^nextcloud_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Nextcloud version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `34.0.3`), but the tags
# do (`v34.0.3-0`). Stripping any `v` before prepending one keeps this correct
